...
import { Company, Investment, Transaction } from '../types';
...
const calculateNewValuation = (company: Company): number => {
  ...
  const totalShares = company.incomingInvestments.reduce(
...
const updateCompanyRanks = (companies: Company[]): Company[] => {
  ...
  return sortedCompanies.map((company, index) => ({
    ...company,
    rank: index + 1,
    valueChange: company.value > 0 ? 
      ((company.value - (company.value / (1 + Math.random() * 0.1))) / company.value) * 100 :
      0
  }));
};
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  invest: async (investorId: string, targetId: string, amount: number) => {
    ...
    const newInvestment: Investment = {
      ...
      timestamp: Date.now(),
      outcome: diceOutcome.shares > 0 ? 
        (diceOutcome.shares === 1 ? 'full' : 'partial') : 
        'negative',
        ...
        if (company.id === investorId) {
          return {
            ...
            cooldownUntil: diceOutcome.timeout > 0 ? 
              Date.now() + (diceOutcome.timeout * 60 * 1000) : // Convert minutes to milliseconds
              undefined,
          };
        }
